...
EuropeanPlainVanilla::EuropeanPlainVanilla(double NoOfPaths_ ,double strike_, double T_, double r_, OptionType OptionType_, double S0_, double rho_, int NoOfSteps_,
...
// Parallel loop with reduction for sum of payoffs
   ...
   for (int i = 0; i < NoOfPaths; ++i) {
    ...
    payoff_call += max(payoff.back() - strike, 0.0) * exp(-r * T);
    payoff_put += max(strike - payoff.back(), 0.0) * exp(-r * T);
}


    switch(Option_Type) {
        case Call:
        return payoff_call / NoOfPaths;
        case Put:
        return payoff_put / NoOfPaths;
        default:
